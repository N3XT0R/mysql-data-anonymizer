...
class AnonymizerDeprecated
{
    ...
    public function __construct($generator = null, string $configPath = '')
    {
        ...
        $this->mysql_pool = Mysql\pool(Mysql\ConnectionConfig::fromString("host=" . $this->config['DB_HOST'] . ";user=" . $this->config['DB_USER'] . ";pass=" . $this->config['DB_PASSWORD'] . ";db=" . $this->config['DB_NAME']),
            $this->config['NB_MAX_MYSQL_CLIENT']);
    ...
    protected function getSelectData($table, $blueprint)
    {
        ...
        if (!($columns ?? false)) {
            $blueprint->primary = [$blueprint->table . '.' . $blueprint->primary[0]];
